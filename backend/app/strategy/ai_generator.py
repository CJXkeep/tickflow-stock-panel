...
职责: 接收用户自然语言描述 → 读取 docs/策略开发指南.md → 调用 LLM → 返回策略代码。
...
# 策略开发文档路径
GUIDE_PATH = Path(__file__).resolve().parent.parent.parent.parent / "docs" / "策略开发指南.md"
    ...
    def _get_guide(self) -> str:
        if self._guide_cache is None:
            if GUIDE_PATH.exists():
                ...
                logger.warning("策略开发指南.md not found at %s", GUIDE_PATH)
                self._guide_cache = ""
