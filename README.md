...
**[核心功能](#-核心功能)** · **[产品聚焦](./docs/产品聚焦与减法路线.md)** · **[快速开始](#-快速开始)** · **[配置](#️-配置)** · **[路线图](#-路线图)**
...
| 方式 | 说明 | 前提 |
| :--- | :--- | :--- |
| **🤖 AI 生成** | 用自然语言描述策略思路,LLM 读取 [策略开发指南.md](./docs/策略开发指南.md) 自动生成完整 Polars 策略文件(经 `ast` 安全校验,限定 `import polars as pl`)。生成后落入 `data/strategies/ai/`,即刻可用 | 需先在 [配置](#️-配置) 中填入 AI Key |
| **📝 代码自定义 / 策略迁移** | 参照 [策略开发指南](./docs/策略开发指南.md) 的文件结构模板,把你**已有的自有策略**改写为 Polars 文件放入 `data/strategies/custom/`(文件名/ID 建议 `custom_时间戳`),引擎自动发现加载——**轻松迁移你现成的量化项目策略**,无需从头重写 | 无 |
...
- **自然语言 → 策略代码**:用一句话描述策略思路,LLM 读取 `docs/策略开发指南.md` 生成完整 Polars 策略文件
...
- [系统设计评估与演进方案.md](./docs/系统设计评估与演进方案.md) —— 系统设计评估、架构边界与演进路线
- [开发路书.md](./docs/开发路书.md) —— 分阶段开发顺序、验收标准与下一步执行清单
- [产品聚焦与减法路线.md](./docs/产品聚焦与减法路线.md) —— 产品边界、减法原则与复盘工作台方向
- [免费数据源替换方案.md](./docs/免费数据源替换方案.md) —— AkShare 免费数据源改造路线
- [数据契约与缓存刷新.md](./docs/数据契约与缓存刷新.md) —— 本地数据表、字段口径、写入与缓存刷新契约
- [数据源与能力门控契约.md](./docs/数据源与能力门控契约.md) —— TickFlow / AkShare 数据源与能力门控契约
- [总计划与投资进度设计.md](./docs/总计划与投资进度设计.md) —— 总计划、持仓进度、目标曲线与静默健康度设计
- [10年做到150万投资目标规划.md](./docs/计划/10年做到150万投资目标规划.md) —— 长期目标规划样例
- [策略开发指南.md](./docs/策略开发指南.md) —— 策略开发指南(AI 生成器与手写策略的规范)
- [策略构建步骤1-生成完整策略.md](./docs/策略构建步骤1-生成完整策略.md) / [策略构建步骤2-修改策略代码.md](./docs/策略构建步骤2-修改策略代码.md) —— 策略构建提示词
- [强势反包策略创建示例.md](./docs/强势反包策略创建示例.md) —— 策略创建示例
